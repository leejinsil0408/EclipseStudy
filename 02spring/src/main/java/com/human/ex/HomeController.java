...
public class HomeController {
	...
	//http://localhost:8082/formdata/board/checkId?id=abcd&pw=1234
		...
		public String checkIdGet(@RequestParam("id") String id
				,@RequestParam(value="pw", required=false, defaultValue="0") int pw,Model model) {
		//400에러 : pw int라 문자열 넣으면 발생 
		...
		public String checkIdPOST(@RequestParam("id") String id
				,@RequestParam(value="pw", required=false, defaultValue="0") int pw,Model model) {
		//400에러 : pw int라 문자열 넣으면 발생 
	...
	//http://localhost:8081/formdata/member/join?name=10&id=20&pw=30&email=123a
		...
		public String joinData(@RequestParam("name") String name
			...
			?name=park&id=id&pw=pw&email=email
